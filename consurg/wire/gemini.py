...
MCP_WRAPPER_SCRIPT = '''\
#!/usr/bin/env python3
"""Context Surgeon MCP wrapper for Gemini CLI.

Auto-generated by `consurg wire gemini`. Intercepts tool calls and enforces scope.
"""
import json
import sys
from pathlib import Path
...
class GeminiWirer(BaseWirer):
    ...
    def wire(self) -> WireResult:
        ...
        if "consurg" in config["mcpServers"]:
            return WireResult(
                success=True,
                message="Already wired to Gemini CLI",
        ...
        return WireResult(
            success=True,
            message="Wired to Gemini CLI (MCP server wrapper)",
            config_path=config_path,
        )
    ...
    def unwire(self) -> WireResult:
        ...
        return WireResult(success=True, message="Unwired from Gemini CLI")
